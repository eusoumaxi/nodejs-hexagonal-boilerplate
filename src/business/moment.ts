 ...
 */
const momentWithTz = (dta?: MomentInput, timezone: string = momentConfig.timezone): Moment => {
  if (!isValidEntry(dta)) {
 ...
 */
const getDateFormatted = (dta: Moment): string => {
 ...
 */
const toISOString = (dta?: Moment, timezone: string = momentConfig.timezone): string => {
 ...
 */
const isValidEntry = (dta?: MomentInput): boolean => {
  return R.not(R.isNil(dta)) &&
  moment(dta, moment.ISO_8601).isValid()
}

/**
 * Centralize moment configs
 */
export { momentWithTz, toISOString, getDateFormatted, isValidEntry }
